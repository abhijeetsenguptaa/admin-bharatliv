...
const PostSeriesVideoServices = require('../services/seriesVideo/postSeriesVideo.service');
const SeriesVideoModel = require('../models/seriesVideo.model');
const DeleteSeriesVideoService = require('../services/seriesVideo/deleteSeriesVideo.service');
const HandleSeriesVideoStatusService = require('../services/seriesVideo/handleSeriesVideoStatus.service');
const GetSeriesVideoService = require('../services/seriesVideo/getSeriesVideo.service');
...
async function PostSeriesVideoController(req, res) {
    try {
        const { seriesID, title, status, rating } = req.body;
        ...
        const contentUploader = await PostSeriesVideoServices(seriesID, title, thumbNail, video, status, rating);
...
async function GetSeriesVideoController(req, res) {
    try {
        const { id, status, seriesID } = req.query;

        const ContentData = await GetSeriesVideoService(id, status, seriesID);
...
async function DeleteSeriesVideoController(req, res) {
    try {
        const id = req.params.id;
        const data = await SeriesVideoModel.findByPk(id);

        const thumbNailKey = data.thumbNail.split('/').pop();
        await s3.deleteObject({ Bucket: BUCKET_NAME, Key: 'videos/' + thumbNailKey }).promise();

        const videoKey = data.video.split('/').pop();
        await s3.deleteObject({ Bucket: BUCKET_NAME, Key: 'videos/' + videoKey }).promise();

        const deleteVideo = await DeleteSeriesVideoService(id);
...
async function HandleStatusSeriesVideoController(req, res) {
    try {
        const id = req.params.id;

        const handleContent = await HandleSeriesVideoStatusService(id);
...
module.exports = { upload, PostSeriesVideoController, GetSeriesVideoController, EditContentController, DeleteSeriesVideoController, HandleStatusSeriesVideoController };